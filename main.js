...
// 更新逻辑
function update() {
  // 更新世界（植物再生，传入摄像机信息动态扩展世界）
  if (world) {
    world.update(camera, canvas.width, canvas.height);
  }
  ...
  // 对所有生物进行动态边界检测
  if (world) {
    const bounds = world.viewBounds;
    const padding = 10;
    for (const creature of creatures) {
      if (!creature.isAlive) continue;

      // 左边界
      if (creature.x < bounds.minX + padding) {
        creature.x = bounds.minX + padding;
        creature.vx = Math.abs(creature.vx);
      }
      // 右边界
      if (creature.x > bounds.maxX - padding) {
        creature.x = bounds.maxX - padding;
        creature.vx = -Math.abs(creature.vx);
      }
      // 上边界
      if (creature.y < bounds.minY + padding) {
        creature.y = bounds.minY + padding;
        creature.vy = Math.abs(creature.vy);
      }
      // 下边界
      if (creature.y > bounds.maxY - padding) {
        creature.y = bounds.maxY - padding;
        creature.vy = -Math.abs(creature.vy);
      }
    }
  }
  ...
  // 处理能量耗尽的生物死亡（并生成植物）
  for (const creature of creatures) {
    if (creature.isAlive && DeathUtils.shouldDie(creature)) {
      DeathUtils.handleDeath(creature, world);
    }
  }
